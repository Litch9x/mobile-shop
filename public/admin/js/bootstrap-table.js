...
!(function ($) {
  ...
  BootstrapTable.prototype.initHeader = function () {
    ...
    $.each(this.options.columns, function (i, column) {
      ...
      style += sprintf(
        "width: %spx; ",
        column.checkbox || column.radio ? 36 : column.width,
      );
      ...
      text = column.title;
      if (
        that.options.sortName === column.field &&
  ...
  BootstrapTable.prototype.initToolbar = function () {
    ...
    if (this.options.showRefresh) {
      html.push(
        sprintf(
          '<button class="btn btn-default" type="button" name="refresh" title="%s">',
    ...
    if (this.options.showToggle) {
      html.push(
        sprintf(
          '<button class="btn btn-default" type="button" name="toggle" title="%s">',
    ...
    if (this.options.showColumns) {
      html.push(
        sprintf(
          '<div class="keep-open %s" title="%s">',
          this.options.showRefresh || this.options.showToggle
            ? "btn-group"
            : "",
  ...
  BootstrapTable.prototype.initPagination = function () {
    ...
    $.each(pageList, function (i, page) {
      if (
        that.options.smartDisplay === false ||
        that.options.totalRows >= page ||
        page == pageList[0]
      ) {
    ...
    for (i = from; i <= to; i++) {
      html.push(
        '<li class="page-number' +
          (i === this.options.pageNumber ? " active disabled" : "") +
          '">',
  ...
  BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        if (that.options.columns[j].checkbox || that.options.columns[j].radio) {
          ...
          text = [
            ...
            "<input" +
              ...
              sprintf(
                ' disabled="%s"',
                !that.options.columns[j].checkboxEnabled ||
      ...
      .on("click", function () {
        ...
        if (that.options.clickToSelect) {
          if (that.header.clickToSelects[$tr.children().index($(this))]) {
            $tr
              .find(sprintf('[name="%s"]', that.options.selectItemName))
              .trigger("click");
          }
        }
      });
    ...
    this.$selectItem.off("click").on("click", function (event) {
      ...
      var checkAll =
          that.$selectItem.filter(":enabled").length ===
  ...
  BootstrapTable.prototype.resetHeader = function () {
    var that = this,
      ...
      scrollWidth =
        this.$el.width() > $fixedBody.width() ? getScrollBarWidth() : 0;
    ...
    setTimeout(function () {
      $fixedHeader
        ...
        .find("table")
        .css("width", that.$el.css("width"))
        .html("")
        .attr("class", that.$el.attr("class"))
        .append(that.$header_);
